from flask import *
from app import app, models, db, secret
...
def homepage():
    ...
    user_subscription = models.Subscriptions.query.filter_by(
...
def login_new_user():
    ...
    if existing_user:
        flash(
            f'Welcome back {existing_user.username}. Thank you for resubscribing to our services')
    else:
        flash(
            f'You have been registered and logged in successfully. Welcome {str(user.username)}!')
    return redirect(url_for('homepage'))
...
def pmgs(pmg_id):
    ...
    if creation_form.validate_on_submit():
        ...
        if existing_group:
            ...
            db.session.commit()
            new_pmg = models.PMG(user_id=current_user.id,
                                 group_id=new_pmg_group.id)
...
def generate_group_map(group_id):
    ...
    for gpx_file in gpx_files:
        waypoints = models.GPXWaypoint.query.filter(
            models.GPXWaypoint.file_id.in_([gpx_file.id for gpx_file in gpx_files])).all()
        tracks = models.GPXTrack.query.filter_by(file_id=gpx_file.id).all()
        track_points = models.GPXTrackPoint.query.join(models.GPXTrack).filter(
            models.GPXTrack.file_id == gpx_file.id).all()
...
def viewgroup(group_id):
    ...
    is_pmg = False
    is_pmg = models.PMG.query.filter_by(
        user_id=current_user.id, group_id=group_id).first() is not None
